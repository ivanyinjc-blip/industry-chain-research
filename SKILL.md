...
## 📐 9 段对齐架构(2026-07-06 v4.1:在 8+1 之前加 chain-radar 行业雷达)

```
     ┌──────────────────┐
     │  chain-radar     │ 0 行业雷达:从 1,561 只 ETF 筛出 Top 行业
     │  (新增 · 选赛道) │   (本 skill v4 独创 · 三轴评分:景气/趋势/拥挤)
     └──────┬───────────┘
            ▼
     ┌─────────────────┐
...
## 🛠 9 个核心模块(8 段 + 1 可选 + 总纲 · 2026-07-06 v4.1)

| # | 模块 | 产物 | 输入 | 核心方法论 | 必跑 |
|---|---|---|---|---|---|
| 0 | **总纲**(本文件) | 调度指令 | 用户提问 | 哪一段被激活 | - |
| **0.5** | **chain-radar**(v4.1 新) | radar.md | 用户问"研究哪个行业" | **「景气 × 趋势 × 拥挤度」三轴 + ETF 雷达扫描** | ⚠️ 仅"选赛道"时跑 |
...
- **v3.3 (2026-07-03)**:★ 产业链图生成系统升级为 PIL + cairosvg 高保真版 + 18 个 SVG 图标库(替代 emoji)+ chain-chip-design/SKILL.md 沉淀设计语言 + 复用模板
- **v3.4 (2026-07-03)**:★ icon_lib 升级为 Bootstrap Icons v1.x 实心版(MIT · 19 个图标 · CDN 自动缓存)+ 径向彩色光圈 + 立体描边阴影(参考图风格)+ 已发飞书用户确认
- **v4.0 (2026-07-06)**:★ 新增 chain-settlement(兑现度筛选)7.5 段,基于 12M Ps/Pf 评分 + 4 象限分类
- **v4.1 (2026-07-06)**:★ 新增 chain-radar(行业雷达)第 0 段,基于 1,561 只 ETF 的「景气 × 趋势 × 拥挤度」三轴筛选,问"研究哪个行业"时首选
- **v4 后续预研方向**:
