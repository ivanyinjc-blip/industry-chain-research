#!/usr/bin/env python3
"""
A股数据获取脚本 v3.1 - 字段元数据 + 异常校验(v4.1.1)
数据源:
...
v4.1.1 修复:
- f191/f167 字段单位(原来是错误地除以 100,实际是 PE/PB 倍数)
- 新增 FIELD_META 字段元数据 + validate_valuation() 异常校验
- 输出 JSON 增加 _quality 标记(suspect/warning/ok)
"""
import sys
import json
import re
...
# ============================================================
# 字段元数据 (FIELD_META)
# v4.1.1 新增:声明每个 push2 字段的真实单位、口径、来源
# 写入 docs/data-fields.md 时同步更新
# ============================================================
FIELD_META = {
    "f43":  {"name": "最新价",       "unit": "分",  "desc": "乘 0.01 得元;停牌返回 -1"},
    "f44":  {"name": "最高",         "unit": "分",  "desc": "乘 0.01 得元"},
    "f45":  {"name": "最低",         "unit": "分",  "desc": "乘 0.01 得元"},
    "f46":  {"name": "开盘",         "unit": "分",  "desc": "乘 0.01 得元"},
    "f47":  {"name": "成交量",       "unit": "手",  "desc": "1 手 = 100 股"},
    "f48":  {"name": "成交额",       "unit": "元",  "desc": "无需换算"},
    "f57":  {"name": "代码",         "unit": "str", "desc": "如 600519"},
    "f58":  {"name": "名称",         "unit": "str", "desc": "中文名"},
    "f60":  {"name": "昨收",         "unit": "分",  "desc": "乘 0.01 得元"},
    "f50":  {"name": "量比",         "unit": "倍",  "desc": "无需换算"},
    "f51":  {"name": "涨停价",       "unit": "分",  "desc": "乘 0.01 得元"},
    "f52":  {"name": "跌停价",       "unit": "分",  "desc": "乘 0.01 得元"},
    "f117": {"name": "总市值",       "unit": "元",  "desc": "无需换算;除 1e8 得亿"},
    "f168": {"name": "换手率",       "unit": "%",   "desc": "无需换算"},
    "f167": {"name": "市净率 PB",    "unit": "倍",  "desc": "v4.1.1 修复:原来是除以 100 的错误,实际是 PB 倍数(茅台 ~8)"},
    "f292": {"name": "流通市值",     "unit": "元",  "desc": "无需换算;除 1e8 得亿"},
    "f191": {"name": "市盈率(动)",   "unit": "倍",  "desc": "v4.1.1 修复:原来是除以 100 的错误,实际是 PE 倍数(茅台 ~25);负数=亏损"},
    "f192": {"name": "市净率(备)",   "unit": "倍",  "desc": "f167 备援"},
    "f173": {"name": "ROE",          "unit": "%",   "desc": "无需换算"},
}
FIELD_META_VERSION = "4.1.1"
...
def validate_valuation(pe, pb, net_profit_yi=None):
    """
    估值数据异常校验 v4.1.1

    返回: (quality: str, message: str)
    - "ok"      : 数值正常
    - "warning" : 偏离正常范围(微利股、新股、负利润等),但字段本身没错
    - "suspect" : 字段映射或单位疑似错位,需人工核查
    """
    issues = []
    ...
    if pe is None:
        ...
        if pe < 0 and net_profit_yi and net_profit_yi > 0:
            # 经典 v4.1.0 bug 场景:动态 PE 为负但公司盈利
            # 通常是 f191 字段被错误地除以 100,或者 f192 错位
            return "suspect", (
                f"动态 PE={pe} 为负但公司净利润={net_profit_yi}亿>0, "
                f"疑似 f191 字段错位或单位错误,见 docs/data-fields.md"
            )
        if pe > 500:
            issues.append(f"PE={pe} > 500(微利或一次性损益)")
        if pe < 0:
            issues.append(f"PE={pe} < 0(公司亏损,非错位)")
    ...
    if pb is None:
        ...
        if pb < 0:
            return "suspect", f"PB={pb} 为负,通常字段错位"
        if pb > 30:
            issues.append(f"PB={pb} > 30(高估值/资不抵债重组)")
...
def fetch_quote(code):
    ...
    # === v4.1.1 修复:f191/f167 不再除以 100,直接是 PE/PB 倍数 ===
    # 保留 _raw 字段供调试;_quality 标记字段健康度
    pe_raw = safe_float(d.get("f191"), 0)  # 原始倍数
    pb_raw = safe_float(d.get("f167"), 0)  # 原始倍数
    quality, qmsg = validate_valuation(pe_raw, pb_raw)
    ...
    return {
        ...
        "_meta": {
            "field_meta_version": FIELD_META_VERSION,
            "field_meta": {k: v["unit"] for k, v in FIELD_META.items()},
        },
        "data": {
            "code": code,
            "name": d.get("f58", ""),
            ...
            "pe_dynamic": pe_raw,        # v4.1.1: 直接是倍数,不再除以 100
            "pe_dynamic_raw": pe_raw,    # 保留原始值,供调试/追溯
            "pb": pb_raw,                # v4.1.1: 直接是倍数
            "pb_raw": pb_raw,
            "_pe_quality": quality,      # ok / warning / suspect
            "_pe_quality_msg": qmsg,
